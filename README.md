...
Using CLion
Install CLion (see "Software requirements" below).
Open CLion and choose Open → select this project folder (the folder that contains platformio.ini).
...
Connect the other leg of the LDR to the Analog input pin (A0) and to the top of the 10 kΩ resistor.
Connect the bottom of the 10 kΩ resistor to GND.
This makes a voltage divider; measure the middle node at A0.
...
Quick start (example)
Connect the UNO R4 WiFi via USB.
Open terminal in project root.
Build: pio run
Upload: pio run -e uno_r4_wifi -t upload
Monitor serial output: pio device monitor -p COM11 -b 9600
Troubleshooting
